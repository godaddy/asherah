...
namespace GoDaddy.Asherah.AppEncryption.Tests.AppEncryption.Json
{
    ...
    public class AppJsonEncryptionImplTest : IClassFixture<MetricsFixture>, IClassFixture<ConfigFixture>
    {
        ...
        private BasicExpiringCryptoPolicy cryptoPolicy;

        public AppJsonEncryptionImplTest(ConfigFixture configFixture)
        {
            ...
            keyManagementService = new DummyKeyManagementService(configFixture.Configuration);

            cryptoPolicy = BasicExpiringCryptoPolicy.BuildWithConfiguration(configFixture.Configuration);
            AeadEnvelopeCrypto aeadEnvelopeCrypto = cryptoPolicy.GetCrypto();
        ...
        {
            RoundTripGeneric(testData, cryptoPolicy.GetCrypto());
        }

        private void RoundTripGeneric(string testData, AeadEnvelopeCrypto aeadEnvelopeCrypto)
        {
            CryptoPolicy cryptoPolicy = new DummyCryptoPolicy(configuration);
