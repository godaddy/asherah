...
@Path("/v1/java/securememory")
@Produces(MediaType.APPLICATION_JSON)
public class SecureMemoryResources {
  ...
  private static final String PACKAGE_MULTITHREADED = PACKAGE_BASE + ".securememory.multithreaded";

  public SecureMemoryResources(final ServerConfiguration config) {
    this.config = config;
  ...
  public String runMultithreadedWriteSuite(@Context final UriInfo uriInfo,
      @QueryParam(TEST_PARAM_NUM_REQUESTS) @DefaultValue("1000") final int numRequests,
      @QueryParam(TEST_PARAM_NUM_THREADS) @DefaultValue("50") final int numThreads,
  ...
  public Response runMultithreadedWriteSuiteAsync(@Context final UriInfo uriInfo,
    ...
    CompletableFuture.runAsync(() -> runMultithreadedWriteSuite(uriInfo, numRequests, numThreads, payloadSizeBytes,
