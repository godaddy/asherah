...
pub fn alloc(size: usize) -> Result<&'static mut [u8], MemcallError> {
    let ptr = unsafe {
        libc::mmap(
            ...
            libc::MAP_PRIVATE | libc::MAP_ANON | libc::MAP_NOCORE,
            -1,
            0,
        )
    };
...
pub fn lock(ptr: &mut [u8]) -> Result<(), MemcallError> {
    ...
    unsafe {
        // Advise the kernel not to dump this memory (MADV_NOCORE for FreeBSD)
        libc::madvise(as_mut_ptr(ptr), as_len(ptr), libc::MADV_NOCORE);
    }
...
pub fn set_limit(resource: RlimitResource, value: u64) -> Result<(), MemcallError> {
    ...
    if result != 0 {
        return Err(MemcallError::SystemError(format!(
            ...
            resource_id, // Consider mapping resource_id back to a string for better error
            std::io::Error::last_os_error()
        )));
    }
    Ok(())
