...
pub fn alloc(size: usize) -> Result<&'static mut [u8], MemcallError> {
    // AIX uses MAP_ANON, similar to generic Unix
    let ptr = unsafe {
        libc::mmap(
            ptr::null_mut(),
            size,
...
pub fn lock(ptr: &mut [u8]) -> Result<(), MemcallError> {
    ...
    // AIX Go implementation does not call madvise here.
    ...
    if result != 0 {
        ...
        if e.raw_os_error() == Some(libc::EPERM) {
            msg = format!(
                "<memcall> could not acquire lock on {:p}, do you have PV_ROOT? [Err: {}]",
                ptr.as_ptr(),
                e
            );
        }
        return Err(MemcallError::SystemError(msg));
    }
    Ok(())
}
...
pub fn unlock(ptr: &mut [u8]) -> Result<(), MemcallError> {
    ...
    if result != 0 {
        ...
        if e.raw_os_error() == Some(libc::EPERM) {
            msg = format!(
                "<memcall> could not free lock on {:p}, do you have PV_ROOT? [Err: {}]",
                ptr.as_ptr(),
                e
            );
        }
