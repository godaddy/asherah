...
impl Metastore for RedisMetastore {
    async fn load(&self, id: &str, created: i64) -> appencryption::Result<Option<EnvelopeKeyRecord>> {
        let key = Self::generate_key(id, created);
    ...
    async fn store(&self, id: &str, created: i64, envelope: &EnvelopeKeyRecord) -> appencryption::Result<bool> {
        let key = Self::generate_key(id, created);
        let mut store = self.store.write().unwrap();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Encrypted Alice's data: {} bytes", alice_encrypted.data.len());
    ...
    println!("Alice's decrypted data: {}", String::from_utf8_lossy(&alice_decrypted));
    println!("Bob's decrypted data: {}", String::from_utf8_lossy(&bob_decrypted));
