...
const CONNECTION_STRING: &str = "server=localhost;database=asherah;user id=sa;password=Password123;TrustServerCertificate=true";
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Get a session
    info!("Getting a session for partition: service={}, product={}", service_name, product_id);
...
async fn perform_encryption_operations(session: &impl Session) -> Result<(), Box<dyn Error>> {
    ...
    info!("Data encrypted successfully. Encrypted length: {} bytes", encrypted.len());
    ...
    info!("Data decrypted successfully. Decrypted length: {} bytes", decrypted.len());
    
    // Verify the decryption worked correctly
    assert_eq!(data.as_ref(), decrypted.as_slice(), "Decrypted data doesn't match original data");
