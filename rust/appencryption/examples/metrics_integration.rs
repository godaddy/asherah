...
impl MetricsCollector {
    ...
    fn record_encrypt(&self, duration: Duration) {
        self.encrypt_count.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    fn record_decrypt(&self, duration: Duration) {
        self.decrypt_count.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    fn print_metrics(&self) {
        let encrypt_count = self.encrypt_count.load(std::sync::atomic::Ordering::Relaxed);
        let decrypt_count = self.decrypt_count.load(std::sync::atomic::Ordering::Relaxed);
        let total_encrypt_time = self.total_encrypt_time.load(std::sync::atomic::Ordering::Relaxed);
        let total_decrypt_time = self.total_decrypt_time.load(std::sync::atomic::Ordering::Relaxed);
        
        println!("\nMetrics Summary:");
        println!("  Encryption operations: {}", encrypt_count);
        println!("  Decryption operations: {}", decrypt_count);
        if encrypt_count > 0 {
            println!("  Average encryption time: {}ms", total_encrypt_time / encrypt_count);
        }
        if decrypt_count > 0 {
            println!("  Average decryption time: {}ms", total_decrypt_time / decrypt_count);
        }
    }
}
...
async fn perform_encryption_operations(
    ...
    for i in 0..count {
        ...
        println!("  Encrypted {} bytes in {}ms", data.len(), encrypt_duration.as_millis());
        ...
        println!("  Decrypted {} bytes in {}ms", decrypted.len(), decrypt_duration.as_millis());
