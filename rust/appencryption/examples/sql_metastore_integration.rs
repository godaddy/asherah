...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match db_type.as_str() {
        ...
        _ => {
            ...
            {
                eprintln!("No SQL features enabled. Enable either the 'mysql' or 'postgres' feature.");
                std::process::exit(1);
            }
            #[cfg(any(feature = "mysql", feature = "postgres"))]
            {
                eprintln!("Unknown database type: {}. Use DB_TYPE=mysql or DB_TYPE=postgres", db_type);
                std::process::exit(1);
            }
        }
    }
