...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data = b"This is sensitive data that will be encrypted using AWS KMS for envelope encryption".to_vec();
    
    // Encrypt data (this will use the KMS client)
    let encrypted = session.encrypt(&data).await?;
    println!("Successfully encrypted data: {} bytes", encrypted.data.len());
    println!("Encrypted with key: {}", encrypted.key.id);
    
    // Decrypt the data
    let decrypted = session.decrypt(&encrypted).await?;
    println!("Successfully decrypted data: {}", String::from_utf8_lossy(&decrypted));
    ...
    println!("Successfully decrypted west data in east region: {}", 
