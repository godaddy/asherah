...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    #[cfg(feature = "aws-v2-dynamodb")]
    {
        ...
        let region = config.region().map(|r| r.to_string()).unwrap_or_else(|| "us-west-2".to_string());
        ...
        arn_map.insert(
            "us-west-2".to_string(),
            "arn:aws:kms:us-west-2:123456789012:key/abcd1234-a123-456a-a12b-a123b4cd56ef".to_string()
        );
        arn_map.insert(
            "us-east-1".to_string(),
            "arn:aws:kms:us-east-1:123456789012:key/abcd1234-a123-456a-a12b-a123b4cd56ef".to_string()
        );
