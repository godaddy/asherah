...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let policy_configurations = vec![
        ("Fast expiry (1 hour)", 
         CryptoPolicy::new()
             .with_expire_after(std::time::Duration::from_secs(60 * 60))),
        ("Standard expiry (24 hours)", 
         CryptoPolicy::new()
             .with_expire_after(std::time::Duration::from_secs(60 * 60 * 24))),
        ("Slow expiry (7 days)", 
         CryptoPolicy::new()
             .with_expire_after(std::time::Duration::from_secs(60 * 60 * 24 * 7))),
    ];
    ...
    let optimized_factory = create_optimized_factory(
        kms.clone(),
        metastore.clone(),
        secret_factory.clone(),
    );
...
fn create_optimized_factory(
    ...
    let policy = CryptoPolicy::new()
        .with_expire_after(std::time::Duration::from_secs(60 * 60 * 24));    // Keys expire after 24 hours
...
async fn benchmark_sequential(
    ...
    for data in test_data.iter() {
        ...
        println!("    Data size: {} bytes - {:.2} ops/sec ({:.2} ms/op)",
            data_size,
            ops_per_sec,
            1000.0 / ops_per_sec);
    }
...
async fn benchmark_high_throughput(
    ...
    println!("    Throughput: {:.2} ops/sec, {:.2} MB/sec",
        ops_per_sec,
        throughput_mbps);
