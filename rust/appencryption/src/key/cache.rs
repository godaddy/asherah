...
pub trait KeyCacher: Send + Sync {
    ...
    async fn get_or_load<F, Fut>(&self, meta: KeyMeta, loader: F) -> Result<Arc<CachedCryptoKey>>
    where
    ...
    async fn get_or_load_latest<F, Fut>(&self, id: &str, loader: F) -> Result<Arc<CachedCryptoKey>>
    where
        F: FnOnce(KeyMeta) -> Fut + Send,
        Fut: std::future::Future<Output = Result<CryptoKey>> + Send;
...
impl KeyCache {
    ...
    fn is_reload_required(&self, entry: &CacheEntry) -> bool {
        ...
        let check_interval =
            Duration::from_std(self.policy.revoke_check_interval).unwrap_or_default();
