...
pub fn set_metrics_provider(provider: Box<dyn MetricsProvider>) {
    let mut global_provider = METRICS_PROVIDER
        .write()
        .expect("Failed to acquire write lock on global metrics provider");
...
pub fn register_counter(name: &str) {
    if metrics_enabled() {
        if let Some(provider) = METRICS_PROVIDER
            .read()
            .expect("Failed to acquire read lock on global metrics provider")
            .as_ref()
        {
    ...
    impl MetricsProvider for PrometheusMetricsProvider {
        ...
        fn register_counter(&self, name: &str) {
            let mut counters = self
                .counters
                .lock()
                .expect("Failed to acquire lock on counters map");
        ...
        fn register_gauge(&self, name: &str) {
            let mut gauges = self
                .gauges
                .lock()
                .expect("Failed to acquire lock on gauges map");
        ...
        fn register_timer(&self, name: &str) {
            let mut timers = self
                .timers
                .lock()
                .expect("Failed to acquire lock on timers map");
