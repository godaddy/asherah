...
mod tests {
    ...
    impl AwsKmsClient for MockKmsClient {
        async fn encrypt(&self, key_id: &str, plaintext: &[u8]) -> Result<Vec<u8>> {
            // Track calls
            self.encrypt_calls.lock().unwrap().push((key_id.to_string(), plaintext.to_vec()));
        ...
        async fn decrypt(&self, key_id: &str, ciphertext: &[u8]) -> Result<Vec<u8>> {
            // Track calls
            self.decrypt_calls.lock().unwrap().push((key_id.to_string(), ciphertext.to_vec()));
        ...
        async fn generate_data_key(&self, key_id: &str) -> Result<GenerateDataKeyResponse> {
            // Track calls
            self.generate_data_key_calls.lock().unwrap().push(key_id.to_string());
    ...
    async fn test_with_endpoint_detailed() {
        ...
        assert_eq!(*endpoint_seen.lock().unwrap(), Some(custom_endpoint.to_string()));
    ...
    async fn test_new_aws_kms_convenience_function_detailed() {
        ...
        arn_map.insert(region.to_string(), format!("arn:aws:kms:{}:123456789012:key/abcd-1234", region));
        ...
        let builder = AwsKmsBuilder::new(crypto.clone(), arn_map.clone())
            .with_kms_factory(factory);
        ...
        let encrypted = kms.encrypt_key(&test_data).await.expect("Failed to encrypt");
        let decrypted = kms.decrypt_key(&encrypted).await.expect("Failed to decrypt");
