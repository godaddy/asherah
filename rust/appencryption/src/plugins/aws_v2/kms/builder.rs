...
use crate::plugins::aws_v2::kms::client::{AwsKms, AwsKmsClient, RegionalClient, StandardAwsKmsClient};
use std::sync::Arc;
...
fn default_kms_factory(config: SdkConfig) -> Arc<dyn AwsKmsClient> {
    let region = config.region().map(|r| r.as_ref().to_string()).unwrap_or_else(|| "us-east-1".to_string());
...
impl AwsKmsBuilder {
    ...
    pub fn with_timeout(mut self, timeout: Duration) -> Self {
        self.timeout_config = Some(TimeoutConfig::builder()
    ...
    pub async fn build(mut self) -> Result<AwsKms> {
        ...
        let factory = self.factory.clone().unwrap_or_else(|| Arc::new(default_kms_factory));
        ...
        if let Some(arn) = self.arn_map.get(&preferred_region) {
            let regional_client = self.create_regional_client(&preferred_region, arn, &sdk_config, &factory).await?;
        ...
        for (region, arn) in &self.arn_map {
            ...
            let regional_client = self.create_regional_client(region, arn, &sdk_config, &factory).await?;
            regional_clients.push(regional_client);
        }
