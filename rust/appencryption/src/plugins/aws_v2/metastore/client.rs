...
impl MultiRegionClient {
    ...
    async fn check_health(&self, client: &Arc<dyn DynamoDbClient>) -> bool {
        ...
        if !self.needs_recheck(client) {
            let health = self.health.read().unwrap();
            return health.get(client.region())
                .map(|h| h.is_healthy())
                .unwrap_or(true);
        }
...
impl Metastore for DynamoDbMetastore {
    async fn load(&self, id: &str, created: i64) -> Result<Option<EnvelopeKeyRecord>> {
        ...
        let key = key.clone();
        let item = self.client.execute_async(move |client| {
            let client = client.clone();
            let table = table.clone();
            let key = key.clone();
            async move {
                client.get_item(&table, key).await
            }
        }).await?;
    ...
    async fn load_latest(&self, id: &str) -> Result<Option<EnvelopeKeyRecord>> {
        ...
        let items = self.client.execute_async(move |client| {
            let client = client.clone();
            let table = table.clone();
            let id = id.clone();
            async move {
                client.query_latest(&table, &id).await
            }
        }).await?;
    ...
    async fn store(&self, id: &str, created: i64, envelope: &EnvelopeKeyRecord) -> Result<bool> {
        ...
        let table = self.table_name.clone();
        let item = item.clone();
        let result = self.client.execute_async(move |client| {
            let client = client.clone();
            let table = table.clone();
            let item = item.clone();
            async move {
                client.put_item_if_not_exists(&table, item).await
            }
        }).await;
