...
use crate::plugins::aws_v2::metastore::client::{DynamoDbClient, DynamoDbKey, DynamoDbItem, PARTITION_KEY, SORT_KEY, KEY_RECORD};
...
impl StandardDynamoDbClient {
    ...
    fn build_key_attributes(key: &DynamoDbKey) -> HashMap<String, AttributeValue> {
        ...
        attributes.insert(
            SORT_KEY.to_string(),
            Self::to_number_attribute(key.created),
        );
...
impl DynamoDbClient for StandardDynamoDbClient {
    async fn get_item(&self, table_name: &str, key: DynamoDbKey) -> Result<Option<DynamoDbItem>> {
        ...
        let key_record_json = item.get(KEY_RECORD)
    ...
    async fn put_item_if_not_exists(&self, table_name: &str, item: DynamoDbItem) -> Result<bool> {
        ...
        key_attributes.insert(
            "Id".to_string(),
            Self::to_string_attribute(item.id.clone()),
        );
            ...
            .await {
                ...
                Err(err) => {
                    ...
                    if err.to_string().contains("ConditionalCheckFailedException") {
                        ...
                        Err(Error::Metastore(format!("DynamoDB put_item error: {}", err)))
                    }
                }
            }
    }
    ...
    async fn health_check(&self) -> Result<bool> {
        ...
        match self.client.describe_endpoints()
            .send()
            .await {
                Ok(_) => {
