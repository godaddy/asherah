...
mod client;
pub use client::{DynamoDbClient, StandardDynamoDbClient, DynamoDbConfig};
pub use client::{DynamoDbKey, DynamoDbItem, DynamoDbEnvelope, DynamoDbKeyMeta};
...
impl Metastore for DynamoDbMetastore {
    ...
    async fn load_latest(&self, id: &str) -> Result<Option<EnvelopeKeyRecord>> {
        ...
        let items = self.client.query_latest(&self.table_name, &id_with_suffix).await?;
    ...
    async fn store(&self, id: &str, created: i64, envelope: &EnvelopeKeyRecord) -> Result<bool> {
        ...
        self.client.put_item_if_not_exists(&self.table_name, item).await
    }
