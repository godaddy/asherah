...
const DEFAULT_LOAD_KEY_QUERY: &str = "SELECT key_record FROM encryption_key WHERE id = ? AND created = ?";
const DEFAULT_STORE_KEY_QUERY: &str = "INSERT INTO encryption_key (id, created, key_record) VALUES (?, ?, ?)";
const DEFAULT_LOAD_LATEST_QUERY: &str = "SELECT key_record from encryption_key WHERE id = ? ORDER BY created DESC LIMIT 1";
...
pub trait SqlClient: Send + Sync {
    ...
    async fn load_key(&self, query: &str, id: &str, created: DateTime<Utc>) -> Result<Option<String>>;
    
    /// Loads the latest key record by ID
    async fn load_latest_key(&self, query: &str, id: &str) -> Result<Option<String>>;
    
    /// Stores a key record
    async fn store_key(&self, query: &str, id: &str, created: DateTime<Utc>, key_record: &str) -> Result<bool>;
}
...
impl Metastore for SqlMetastore {
    async fn load(&self, id: &str, created: i64) -> Result<Option<EnvelopeKeyRecord>> {
        ...
        let result = match self.client.load_key(&self.load_key_query, id, created_dt).await? {
    ...
    async fn load_latest(&self, id: &str) -> Result<Option<EnvelopeKeyRecord>> {
        ...
        let result = match self.client.load_latest_key(&self.load_latest_query, id).await? {
            Some(json_str) => Some(Self::parse_envelope(&json_str)?),
            None => None,
        };
    ...
    async fn store(&self, id: &str, created: i64, envelope: &EnvelopeKeyRecord) -> Result<bool> {
        ...
        let result = self.client.store_key(&self.store_key_query, id, created_dt, &json_str).await;
