...
impl EnvelopeEncryptionBuilder {
    ...
    pub fn build(self) -> Result<EnvelopeEncryption> {
        let partition = self.partition.ok_or_else(|| Error::InvalidArgument("partition is required".to_string()))?;
        let metastore = self.metastore.ok_or_else(|| Error::InvalidArgument("metastore is required".to_string()))?;
        let kms = self.kms.ok_or_else(|| Error::InvalidArgument("kms is required".to_string()))?;
        let policy = self.policy.ok_or_else(|| Error::InvalidArgument("policy is required".to_string()))?;
        let crypto = self.crypto.ok_or_else(|| Error::InvalidArgument("crypto is required".to_string()))?;
        let secret_factory = self.secret_factory.ok_or_else(|| Error::InvalidArgument("secret_factory is required".to_string()))?;
        let sk_cache = self.sk_cache.ok_or_else(|| Error::InvalidArgument("sk_cache is required".to_string()))?;
        let ik_cache = self.ik_cache.ok_or_else(|| Error::InvalidArgument("ik_cache is required".to_string()))?;
...
impl EnvelopeEncryption {
            ...
            async move {
                let (key_record, created) = if meta.is_latest() {
                    ...
                    if let Some(record) = metastore.load_latest(&meta.id).await? {
                        ...
                        (None, crate::policy::new_key_timestamp(policy.create_date_precision))
                    }
                } else {
                    // Load specific key
                ...
                if let Some(record) = key_record {
                    ...
                    let key_bytes = if is_system_key {
                        ...
                        return Err(Error::Internal("Parent key required for intermediate key decryption".into()));
                    };
                    
                    // Create the crypto key
                    let crypto_key = CryptoKey::new(meta.id.clone(), created, key_bytes, secret_factory.as_ref())?;
                    ...
                    let mut crypto_key = CryptoKey::new(meta.id.clone(), created, key_bytes, secret_factory.as_ref())?;
                    ...
                    let encrypted_key = if is_system_key {
                        ...
                        crypto_key.with_bytes(|key_bytes| -> Result<Vec<u8>> {
                            parent.with_bytes(|parent_bytes| {
                                crypto.encrypt(key_bytes, parent_bytes)
                            })
                        })?
                    } else {
                        return Err(Error::Internal("Parent key required for intermediate key encryption".into()));
                    };
                    ...
                    if !metastore.store(&meta.id, created, &key_record).await? {
                        ...
                        let record = metastore.load(&meta.id, created).await?
                            .ok_or_else(|| Error::Internal("Failed to load key after creation conflict".into()))?;
                        ...
                        crypto_key = CryptoKey::new(meta.id, created, key_bytes, secret_factory.as_ref())?;
                    }
                    
                    Ok(crypto_key)
                } else {
                    Err(Error::KeyNotFound(format!("Key {}:{} not found", meta.id, meta.created)))
                }
            }
        };
    ...
    async fn get_intermediate_key(&self, system_key: Arc<CryptoKey>) -> Result<Arc<crate::key::cache::CachedCryptoKey>> {
        let intermediate_key_id = self.partition.intermediate_key_id();
        
        // Get metrics timer
        let _timer = crate::timer!("ael.envelope.get_intermediate_key");
        
        self.load_create_key(&intermediate_key_id, false, Some(system_key)).await
    }
    ...
    async fn create_data_row_record(&self, data: &[u8]) -> Result<DataRowRecord> {
        ...
        let intermediate_key = self.get_intermediate_key(system_key.crypto_key.clone()).await?;
        ...
        let encrypted_data_key = intermediate_key.crypto_key.with_bytes(|key_bytes| {
            self.crypto.encrypt(&data_key, key_bytes)
        })?;
    ...
    async fn load_intermediate_key(&self, key_meta: &KeyMeta) -> Result<Arc<crate::key::cache::CachedCryptoKey>> {
        ...
        let loader = move |meta: KeyMeta| {
            ...
            async move {
                // Load the key record
                let record = metastore.load(&meta.id, meta.created).await?
                    .ok_or_else(|| Error::KeyNotFound(format!("Key {}:{} not found", meta.id, meta.created)))?;
                    
                // Decrypt with system key
                let key_bytes = system_key.with_bytes(|sk_bytes| {
                    crypto.decrypt(&record.encrypted_key, sk_bytes)
                })?;
...
impl Encryption for EnvelopeEncryption {
    ...
    async fn decrypt_data_row_record(&self, drr: &DataRowRecord) -> Result<Vec<u8>> {
        ...
        let parent_key_meta = drr.key.parent_key_meta.as_ref()
            .ok_or_else(|| Error::Internal("Missing parent key metadata in data row record".into()))?;
        
        // Validate the intermediate key ID belongs to this partition
        if !self.partition.is_valid_intermediate_key_id(&parent_key_meta.id) {
        ...
        // Decrypt data key
        let data_key = intermediate_key.crypto_key.with_bytes(|key_bytes| {
            self.crypto.decrypt(&drr.key.encrypted_key, key_bytes)
        })?;
...
mod tests {
    ...
    async fn test_envelope_encryption() -> Result<()> {
        ...
        // Create caches
        let sk_cache = crate::key::cache::AnyCache::KeyCache(Arc::new(KeyCache::new(CacheKeyType::SystemKeys, policy.clone())));
        let ik_cache = crate::key::cache::AnyCache::KeyCache(Arc::new(KeyCache::new(CacheKeyType::IntermediateKeys, policy.clone())));
