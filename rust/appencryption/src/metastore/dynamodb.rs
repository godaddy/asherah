...
// Note: This is a placeholder. In production, you would use the aws-sdk-dynamodb crate.
// We need a client trait to mock DynamoDB for testing
#[async_trait]
...
impl MultiRegionClient {
    ...
    where
        F: Fn(&Arc<dyn DynamoDbClient>) -> Result<R> + Send + Sync,
    {
        // First try healthy clients
        let clients = self.healthy_clients();

        if clients.is_empty() {
            // No healthy clients, try all clients
            let clients = self.all_clients();

            if clients.is_empty() {
                return Err(Error::Metastore("No DynamoDB clients available".into()));
            }

            // Try all clients with health check if needed
            ...
            return Err(Error::Metastore("All DynamoDB clients failed".into()));
        }

        // Try healthy clients first
        for client in clients {
            match operation(&client) {
                Ok(result) => return Ok(result),
                Err(e) => {
        ...
        // All healthy clients failed, try rechecking all clients
...
impl DynamoDbMetastore {
    ...
    fn decode_item(&self, item: DynamoDbItem) -> Result<EnvelopeKeyRecord> {
        ...
        let mut ekr = EnvelopeKeyRecord {
            ...
            encrypted_key: base64::decode(&item.key_record.encrypted_key)
...
impl Metastore for DynamoDbMetastore {
    ...
    /// Return the region suffix used by this metastore (for SuffixedPartition)
    fn get_region_suffix(&self) -> Option<String> {
