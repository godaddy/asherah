...
impl SessionFactory {
    ...
    pub fn new(
        ...
        let intermediate_keys = if policy.shared_intermediate_key_cache {
            Some(crate::key::cache::AnyCache::KeyCache(Arc::new(
                crate::key::cache::KeyCache::new(
        ...
        let session_cache = if policy.cache_sessions {
            ...
            let loader = move |id: &str| -> Result<Arc<EnvelopeSession>> {
                ...
                let ik_cache = if let Some(shared_ik_cache) = &factory_intermediate_keys {
                    ...
                    crate::key::cache::AnyCache::KeyCache(Arc::new(
                        crate::key::cache::KeyCache::new(
                            crate::key::cache::CacheKeyType::IntermediateKeys,
    ...
    pub async fn session(&self, partition_id: impl Into<String>) -> Result<Arc<EnvelopeSession>> {
        ...
        let partition = Arc::new(DefaultPartition::new(&id, &self.service, &self.product));
...
impl SessionFactoryBuilder {
    ...
    pub fn with_option(
        mut self,
        option: impl Fn(&mut SessionFactory) + Send + Sync + 'static,
    ) -> Self {
    ...
    pub fn with_metrics(mut self, enabled: bool) -> Self {
        self.options
            .push(Box::new(move |_factory: &mut SessionFactory| {
                if !enabled {
    ...
    pub fn build(self) -> Result<SessionFactory> {
        let service = self
            .service
            .ok_or_else(|| Error::InvalidArgument("service name is required".to_string()))?;
        let product = self
            .product
            .ok_or_else(|| Error::InvalidArgument("product name is required".to_string()))?;
        let policy = self
            .policy
            .ok_or_else(|| Error::InvalidArgument("crypto policy is required".to_string()))?;
        let kms = self.kms.ok_or_else(|| {
            Error::InvalidArgument("key management service is required".to_string())
        })?;
        let metastore = self
            .metastore
        ...
        let secret_factory = self
            .secret_factory
            .ok_or_else(|| Error::InvalidArgument("secret factory is required".to_string()))?;
        ...
        let factory = if let Some(crypto) = self.crypto {
            ...
            let mut f = SessionFactory {
                ...
                secret_factory,
                session_cache: None,
                system_keys: crate::key::cache::AnyCache::NeverCache(Arc::new(
                    crate::key::cache::NeverCache,
                )),
            ...
            if policy.cache_sessions {
                ...
                let loader = move |id: &str| -> Result<Arc<EnvelopeSession>> {
                    ...
                    let ik_cache = if let Some(shared_ik_cache) = &factory_intermediate_keys {
                        ...
                        crate::key::cache::AnyCache::NeverCache(Arc::new(
                            crate::key::cache::NeverCache,
                        ))
                    };
