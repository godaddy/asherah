...
impl DynamoDbTestContext {
    ...
    pub async fn seed_db(&self) {
        ...
        let test_key_id = format!("_IK_{}_{}_{}", 
                                 "partition_1",
                                 "service",
                                 "product");
        
        let system_key_id = format!("_SK_{}_{}",
                                  "service",
                                  "product");
        ...
        let encrypted_key = crypto.encrypt(&dummy_key, &system_key)
        ...
        metastore.store(&test_key_id, self.created_timestamp, &ik_envelope).await
