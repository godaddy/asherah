...
async fn test_concurrent_encrypt_decrypt_with_same_session() {
    ...
    // Get session
    let session = Arc::new(
        factory.session(PARTITION_ID).await
    ...
    for i in 0..NUM_THREADS {
        ...
        let task = task::spawn(async move {
            ...
            for j in 0..OPERATIONS_PER_THREAD {
                ...
                // Encrypt the data
                let drr = session_clone.encrypt(&data_bytes).await
                    .expect("Failed to encrypt data");
                
                // Decrypt the data
                let decrypted = session_clone.decrypt(&drr).await
...
async fn test_concurrent_encrypt_decrypt_with_multiple_sessions() {
    ...
    for i in 0..NUM_THREADS {
        ...
        let task = task::spawn(async move {
            ...
            let session = factory_clone.session(&partition_id).await
                .expect("Failed to get session");
            ...
            for j in 0..OPERATIONS_PER_THREAD {
                ...
                // Encrypt the data
                let drr = session.encrypt(&data_bytes).await
                    .expect("Failed to encrypt data");
