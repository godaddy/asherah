...
impl fmt::Display for MetastoreOperation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            MetastoreOperation::Load { id, created } => write!(f, "Load(id={}, created={})", id, created),
            MetastoreOperation::LoadLatest { id } => write!(f, "LoadLatest(id={})", id),
            MetastoreOperation::Store { id, created } => write!(f, "Store(id={}, created={})", id, created),
        }
    }
}
...
impl<M: Metastore> Metastore for TrackingMetastore<M> {
    async fn load(&self, id: &str, created: i64) -> Result<Option<EnvelopeKeyRecord>, Error> {
        ...
        self.operations.lock().unwrap().push(MetastoreOperation::Load {
            id: id.to_string(),
            created,
        });
    ...
    async fn load_latest(&self, id: &str) -> Result<Option<EnvelopeKeyRecord>, Error> {
        ...
        self.operations.lock().unwrap().push(MetastoreOperation::LoadLatest {
            id: id.to_string(),
        });
    ...
    async fn store(&self, id: &str, created: i64, envelope: &EnvelopeKeyRecord) -> Result<bool, Error> {
        self.store_count.fetch_add(1, Ordering::SeqCst);
        
        self.operations.lock().unwrap().push(MetastoreOperation::Store {
            id: id.to_string(),
            created,
        });
...
async fn test_encrypt_metastore_interactions() {
    ...
    assert!(load_latest_count >= 1, "Should have at least one LoadLatest operation");
    ...
    let has_ik_operations = operations.iter().any(|op| {
        match op {
...
async fn test_decrypt_metastore_interactions() {
    ...
    let inner_session = inner_factory.session(PARTITION_ID).await
        .expect("Failed to get session");
    
    let data = ORIGINAL_DATA.as_bytes().to_vec();
    let drr = inner_session.encrypt(&data).await
        .expect("Failed to encrypt data");
    ...
    assert!(load_count + load_latest_count >= 1, "Should have at least one Load/LoadLatest operation");
    assert_eq!(store_count, 0, "Should not have any Store operations during decrypt");
