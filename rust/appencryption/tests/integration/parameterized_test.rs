...
async fn test_different_policy_configurations() {
    ...
    for config in configurations {
        ...
        if config.name == "short_expiry" {
            policy.expire_key_after = std::time::Duration::from_secs(config.key_expiry_seconds as u64);
        }
        ...
        assert_eq!(ORIGINAL_DATA.as_bytes(), decrypted.as_slice(),
                   "Decryption failed for configuration: {}", config.name);
        ...
        if config.name == "short_expiry" {
            ...
            let new_session = factory.session(PARTITION_ID).await
                .expect("Failed to get session");
            
            // Encrypt with new keys
            let drr2 = new_session.encrypt(&data).await
                .expect("Failed to encrypt data with new keys");
            
            // Verify the new session can decrypt old data
            let decrypted = new_session.decrypt(&drr).await
            ...
            let decrypted = session.decrypt(&drr2).await
