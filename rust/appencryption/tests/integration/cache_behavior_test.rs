...
use crate::integration::common::{
    ...
    PARTITION_ID, ORIGINAL_DATA, PRODUCT, SERVICE, Config,
};
...
impl<M: Metastore> Metastore for CountingMetastore<M> {
    async fn load(&self, id: &str, created: i64) -> Result<Option<appencryption::envelope::EnvelopeKeyRecord>, Error> {
        self.load_count.fetch_add(1, Ordering::SeqCst);
        self.inner.load(id, created).await
    }
    
    async fn load_latest(&self, id: &str) -> Result<Option<appencryption::envelope::EnvelopeKeyRecord>, Error> {
        self.load_latest_count.fetch_add(1, Ordering::SeqCst);
        self.inner.load_latest(id).await
    }
    
    async fn store(&self, id: &str, created: i64, envelope: &appencryption::envelope::EnvelopeKeyRecord) -> Result<bool, Error> {
...
async fn test_lru_cache_behavior() {
    ...
    for i in 0..3 {
        ...
        let session = factory.session(&partition).await
            .expect("Failed to get session");
        
        let data = ORIGINAL_DATA.as_bytes().to_vec();
        let _drr = session.encrypt(&data).await
            .expect("Failed to encrypt data");
        
        // Should have metastore operations for a cache miss
        let (load_count, load_latest_count, store_count) = metastore.get_counts();
        assert!(load_latest_count > 0 || store_count > 0, 
                "Should have metastore operations for partition {}", i);
    }
    ...
    let session3 = factory.session(&partition3).await
        .expect("Failed to get session");
    
    let data = ORIGINAL_DATA.as_bytes().to_vec();
    let _ = session3.encrypt(&data).await
        .expect("Failed to encrypt data");
    ...
    let session0 = factory.session(&partition0).await
        .expect("Failed to get session");
    
    let _ = session0.encrypt(&data).await
        .expect("Failed to encrypt data");
...
async fn test_cache_expiration_behavior() {
    ...
    assert!(load_latest_count2 <= load_latest_count1 && store_count2 <= store_count1, 
    ...
    let _drr3 = session.encrypt(&data).await
        .expect("Failed to encrypt data");
    
    // Should have metastore operations for a cache miss
    let (load_count3, load_latest_count3, store_count3) = metastore.get_counts();
    assert!(load_latest_count3 > 0 || store_count3 > 0, 
...
async fn test_tlfu_cache_behavior() {
    ...
    let session1 = factory.session(&partition1).await
        .expect("Failed to get session");
    
    for _ in 0..5 {
        let _ = session1.encrypt(&data).await
            .expect("Failed to encrypt data");
    }
    ...
    let session2 = factory.session(&partition2).await
        .expect("Failed to get session");
    
    for _ in 0..3 {
        let _ = session2.encrypt(&data).await
            .expect("Failed to encrypt data");
    }
    ...
    metastore.reset_counts();
    let session0b = factory.session(&partition0).await
        .expect("Failed to get session");
    
    let _ = session0b.encrypt(&data).await
        .expect("Failed to encrypt data");
    
    // Should have metastore operations for a cache miss
    let (load_count0, load_latest_count0, store_count0) = metastore.get_counts();
    println!("After trying partition 0 again: load={}, load_latest={}, store={}", 
             load_count0, load_latest_count0, store_count0);
    assert!(load_latest_count0 > 0 || store_count0 > 0, 
            "Should have metastore operations for partition 0 after eviction");
    
    // But partition 1 should still be cached due to high frequency
    metastore.reset_counts();
    let session1b = factory.session(&partition1).await
        .expect("Failed to get session");
    
    let _ = session1b.encrypt(&data).await
        .expect("Failed to encrypt data");
    
    // Should have fewer operations for a cache hit
    let (load_count1, load_latest_count1, store_count1) = metastore.get_counts();
    println!("After trying partition 1 again: load={}, load_latest={}, store={}", 
