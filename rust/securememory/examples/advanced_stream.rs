...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let consumer = thread::spawn(move || {
        ...
        loop {
            ...
            if rx.try_recv().is_ok() {
                println!("Consumer: Received stop signal. Total processed: {} bytes", total_processed);
                break;
            }
            ...
            match consumer_stream.read(&mut buffer) {
                Ok(bytes_read) if bytes_read > 0 => {
                    ...
                    println!("Consumer: Processing {} bytes. Preview: [{}...]", 
                        bytes_read, preview);
        ...
        match consumer_stream.flush() {
            Ok(final_secret) => {
                final_secret.with_bytes(|bytes| {
                    if !bytes.is_empty() {
                        println!("Consumer: Processed final {} bytes from the stream", bytes.len());
        ...
        println!("Consumer: Thread completed. Total processed: {} bytes", total_processed);
        total_processed
    });
    ...
    let producer = thread::spawn(move || {
        ...
        for i in 0..10 {
            ...
            println!("Producer: Writing chunk {} with {} bytes", i + 1, chunk_size);
    ...
    println!("Main: Both threads completed. Total bytes processed: {}", bytes_processed);
