...
impl ProtectedMemorySecretSimple {
    ...
    pub fn new(data: &[u8]) -> Result<Self> {
        ...
        let full_slice = unsafe { 
            std::slice::from_raw_parts_mut(memory_ptr, aligned_size)
        };
    ...
    fn enable_read_access(&self) -> Result<()> {
        ...
        if self.inner.ptr.is_null() {
            return Err(SecureMemoryError::OperationFailed("Memory pointer is null".to_string()));
        }
        
        let mut protection = self.inner.protection.lock()
            .map_err(|_| SecureMemoryError::OperationFailed("Protection lock poisoned".to_string()))?;
...
impl Secret for ProtectedMemorySecretSimple {
    ...
    fn close(&self) -> Result<()> {
        ...
        let mut guard = self.inner.access_mutex.lock()
            .map_err(|_| SecureMemoryError::OperationFailed("Access mutex poisoned".to_string()))?;
        ...
        loop {
            ...
            wait_count += 1;
            if wait_count % 100 == 0 {
                debug!("Waiting for {} accessors to finish (iteration {})", count, wait_count);
            }
            
            guard = self.inner.access_cond.wait(guard)
                .map_err(|_| SecureMemoryError::OperationFailed("Condition variable wait failed".to_string()))?;
        }
...
impl SecretExtensions for ProtectedMemorySecretSimple {
        ...
        F: FnOnce(&[u8]) -> Result<T>,
    {
        ...
        trace!("Access count increased from {} to {}", old_count, old_count + 1);
...
impl<'a> Drop for AccessGuard<'a> {
    fn drop(&mut self) {
        ...
        trace!("Access count decreased from {} to {}", old_count, old_count - 1);
...
impl Drop for SecretInternal {
    fn drop(&mut self) {
        ...
        if !self.freed.load(Ordering::Acquire) && !self.ptr.is_null() && self.capacity > 0 {
            ...
            if self.freed.compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire).is_err() {
