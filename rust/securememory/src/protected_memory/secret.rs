...
use std::sync::{Arc, RwLock, Mutex, Weak, OnceLock, Condvar};
use std::io::Read;
use zeroize::Zeroize;
...
impl SecretExtensions for ProtectedMemorySecret {
        ...
        F: FnOnce(&[u8]) -> Result<T>,
    {
        ...
        {
            let mut guard = self.inner.rw.lock()
                .map_err(|_| SecureMemoryError::OperationFailed("Failed to acquire access lock".to_string()))?;
            ...
            if guard.access_counter == 0 {
                let bytes_guard = self.inner.bytes.read()
                    .map_err(|_| SecureMemoryError::OperationFailed("Failed to acquire read lock".to_string()))?;
                
                if let Some(memory) = bytes_guard.as_ref() {
                    if let Err(e) = memcall::protect(unsafe { memory.as_mut_slice_for_protection() }, MemoryProtection::ReadOnly) {
        ...
        let result = {
            ...
            let memory = guard.as_ref()
                .ok_or(SecureMemoryError::SecretClosed)?;
        ...
        {
            let mut guard = self.inner.rw.lock()
                .map_err(|_| SecureMemoryError::OperationFailed("Failed to acquire release lock".to_string()))?;
            ...
            if guard.access_counter == 0 {
                ...
                if let Some(memory) = bytes_guard.as_ref() {
                    if let Err(e) = memcall::protect(unsafe { memory.as_mut_slice_for_protection() }, MemoryProtection::NoAccess) {
...
impl ProtectedMemorySecret {
    ...
    fn close_impl(&self) -> Result<()> {
        let mut guard = self.inner.rw.lock()
            .map_err(|_| SecureMemoryError::OperationFailed("Failed to acquire close mutex".to_string()))?;
        ...
        loop {
            ...
            if guard.access_counter == 0 {
                ...
                let mut bytes_guard = self.inner.bytes.write()
                    .map_err(|_| SecureMemoryError::OperationFailed("Failed to acquire write lock for cleanup".to_string()))?;
            ...
            guard = self.inner.condvar.wait(guard)
                .map_err(|_| SecureMemoryError::OperationFailed("Failed to wait on condvar".to_string()))?;
        }
    }
}
