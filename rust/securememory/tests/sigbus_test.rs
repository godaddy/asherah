...
fn test_high_concurrency_sigbus() {
    ...
    let secret = Arc::new(ProtectedMemorySecretSimple::new(b"concurrent data with high stress").unwrap());
...
fn test_concurrent_access_and_close() {
    let secret = Arc::new(ProtectedMemorySecretSimple::new(b"concurrent access and close").unwrap());
