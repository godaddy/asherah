...
fn test_protected_memory_error_handling() {
    ...
    let result: Result<(), SecureMemoryError> = secret.with_bytes(|_| {
        Err(SecureMemoryError::OperationFailed("test error".to_string()))
    });
    
    assert!(matches!(result, Err(SecureMemoryError::OperationFailed(_))));
    assert_eq!(result.unwrap_err().to_string(), "Secret operation failed: test error");
