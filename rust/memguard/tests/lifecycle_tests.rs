...
fn test_buffer_operations_after_purge() {
    ...
    assert!(matches!(buffer_access_result, Err(memguard::MemguardError::SecretClosed)), 
    ...
    new_buffer.with_data_mut(|data| {
        data[0] = 42;
        Ok(())
    }).expect("Failed to write to new buffer after purge");
    
    new_buffer.with_data(|data| {
...
fn test_enclave_operations_after_purge() {
    ...
    match enclave.open() {
        ...
        Err(e) => {
            panic!("Expected SecretClosed or CryptoError, got different error: {:?}", e);
        },
        Ok(_) => {
    ...
    // (Coffer should be re-initialized as needed)
    let new_enclave = memguard::Enclave::new_random(32).expect("Failed to create new enclave after purge");
    
    // Verify the new enclave works correctly
    let buffer = new_enclave.open().expect("Failed to open new enclave after purge");
    assert_eq!(buffer.size(), 32, "New enclave has incorrect size");
    
    buffer.with_data(|data| {
        // Should contain random data (not all zeros)
        assert!(data.iter().any(|&x| x != 0), "Enclave data should be random, not all zeros");
        Ok(())
    }).expect("Failed to read data from new enclave's buffer");
    
    // Clean up
    buffer.destroy().expect("Failed to destroy buffer from new enclave");
}
...
fn test_stream_operations_after_purge() {
    ...
    stream.write_all(initial_data).expect("Failed to write initial data to stream");
    ...
    new_stream.write_all(test_data).expect("Failed to write to new stream after purge");
    
    // Flush the stream to get the buffer with all data
    let (buffer, io_err) = new_stream.flush().expect("Failed to flush new stream after purge");
    assert!(io_err.is_none(), "I/O error during new stream flush");
    
    // Verify the data in the buffer
    buffer.with_data(|data| {
        assert_eq!(data, test_data, "Data in flushed buffer doesn't match written data");
        Ok(())
    }).expect("Failed to read data from new stream's buffer");
    
    // Clean up
    buffer.destroy().expect("Failed to destroy buffer from new stream");
}