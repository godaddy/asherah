...
use memguard::{Buffer, Stream, scramble_bytes, wipe_bytes, MemguardError};
use std::io::Write;
...
fn test_concurrent_buffer_clone() {
    ...
    buffer.with_data(|data| {
        buffer_clone.with_data(|clone_data| {
    ...
    buffer.destroy().expect("Original buffer destruction failed");
    assert!(!buffer.is_alive());
    ...
    buffer_clone.destroy().expect("Clone buffer destruction failed");
}
...
fn test_concurrent_buffer_random() {
    ...
    let is_all_zeroes = random_buffer.with_data(|data| {
        Ok(data.iter().all(|&x| x == 0))
    ...
    random_buffer.melt().expect("Making random buffer mutable failed");
    random_buffer.with_data_mut(|data| {
        data[0] = 0xFF;
        Ok(())
    }).expect("Writing to melted random buffer failed");
    
    random_buffer.destroy().expect("Random buffer destruction failed");
}
...
fn test_concurrent_utility_functions() {
    ...
    // Test wipe_bytes
    wipe_bytes(&mut data);
    assert!(data.iter().all(|&x| x == 0), "Data should be wiped to all zeroes");
}
