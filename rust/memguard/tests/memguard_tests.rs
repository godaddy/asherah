use memguard::{purge, scramble_bytes, wipe_bytes, Buffer, Enclave, MemguardError};
...
fn test_enclave_basic() {
    ...
    assert_ne!(
        data, data_copy,
        "Original data was not wiped after enclave creation"
    );
...
fn test_multiple_enclaves() {
    ...
    for mut buffer_instance in buffers_orig {
        // Iterate and consume original buffers
        let enclave = Enclave::seal(&mut buffer_instance).expect("Sealing buffer failed");
        assert!(
            !buffer_instance.is_alive(),
            "Buffer should be destroyed after sealing"
        );
            ...
            .with_data(move |data| {
                for i in 0..data.len() {
                    assert_eq!(
                        data[i],
                        (i * current_j) as u8,
                        "Data mismatch in unsealed enclave"
                    );
                }
                Ok(())
            })
...
fn test_public_scramble_bytes() {
    ...
    assert_ne!(
        buf, original_buf,
        "buffer not scrambled by public scramble_bytes"
    );
    assert!(
        buf.iter().any(|&x| x != 0),
        "buffer seems to be all zeros after scramble"
    );
}
...
fn test_public_wipe_bytes() {
    ...
    assert!(
        buf.iter().any(|&x| x != 0),
        "buffer should not be all zeros before wipe"
    );
    wipe_bytes(&mut buf);
    assert!(
        buf.iter().all(|&x| x == 0),
        "buffer not wiped by public wipe_bytes"
    );
}

#[test]
fn test_api_new_buffer() {
    // Corresponds to Go's TestNewBuffer
...
#[test]
fn test_api_new_buffer_from_bytes() {
    // Corresponds to Go's TestNewBufferFromBytes
    ...
    .unwrap();
    assert!(
        original_data.iter().all(|&x| x == 0),
        "Source data not wiped"
    );
    ...
    .expect("Write after melt failed");
    ...
    let b0 =
        Buffer::new_from_bytes(&mut empty_data).expect("NewBufferFromBytes with empty data failed");
    assert_eq!(b0.size(), 0, "Size of Buffer from empty bytes should be 0");
    assert!(
        !b0.is_alive(),
        "Buffer from empty bytes should be non-alive (null)"
    );
}

#[test]
fn test_api_new_buffer_from_reader() {
    // Corresponds to Go's TestNewBufferFromReader
    // Case 1: Successful full read
    let data1 = vec![7u8; 128];
    let mut reader1 = Cursor::new(data1.clone());
    let (b1, err1_opt) =
        Buffer::new_from_reader(&mut reader1, 128).expect("NewBufferFromReader case 1 failed");
    ...
    assert!(
        matches!(
            b1.with_data_mut(|_| Ok(())),
            Err(MemguardError::ProtectionFailed(_))
        ),
        "Buffer from reader should be ReadOnly initially"
    );
    b1.destroy().unwrap();
    ...
    let (b2, err2_opt) =
        Buffer::new_from_reader(&mut reader2, 10).expect("NewBufferFromReader case 2 failed");
    assert!(err2_opt.is_some(), "Case 2 should have an I/O error (EOF)");
    if let Some(MemguardError::OperationFailed(msg)) = err2_opt {
        // Changed from IoError to OperationFailed as per new_from_reader impl
        assert!(msg.contains("EOF before filling buffer") || msg.contains("unexpected EOF"));
    } else {
        panic!(
            "Expected OperationFailed EOF error for case 2, got {:?}",
            err2_opt
        );
    }
    ...
    let (b3, err3_opt) =
        Buffer::new_from_reader(&mut reader3, 32).expect("NewBufferFromReader case 3 failed");
    ...
    let (b4, err4_opt) =
        Buffer::new_from_reader(&mut reader4, 0).expect("NewBufferFromReader case 4 failed");
    assert!(
        err4_opt.is_none(),
        "Case 4 should have no I/O error for 0 bytes"
    );
    assert_eq!(b4.size(), 0);
...
#[test]
fn test_api_new_buffer_from_reader_until() {
    // Corresponds to Go's TestNewBufferFromReaderUntil
    // Case 1: Delimiter found
    let data1 = b"hello\nworld".to_vec();
    let mut reader1 = Cursor::new(data1.clone());
    let (b1, err1_opt) = Buffer::new_from_reader_until(&mut reader1, b'\n', None)
        .expect("ReaderUntil case 1 failed");
    ...
    let mut reader2 = Cursor::new(data2.clone());
    let (b2, err2_opt) =
        Buffer::new_from_reader_until(&mut reader2, b'X', None).expect("ReaderUntil case 2 failed");
    ...
    let (b3, err3_opt) =
        Buffer::new_from_reader_until(&mut reader3, b'X', None).expect("ReaderUntil case 3 failed");
    ...
    let (b4, err4_opt) =
        Buffer::new_from_reader_until(&mut reader4, b'X', None).expect("ReaderUntil case 4 failed");
...
#[test]
fn test_api_new_buffer_from_entire_reader() {
    // Corresponds to Go's TestNewBufferFromEntireReader
    // Case 1: Normal read
    let data1 = b"entire content".to_vec();
    let mut reader1 = Cursor::new(data1.clone());
    let (b1, err1_opt) =
        Buffer::new_from_entire_reader(&mut reader1).expect("EntireReader case 1 failed");
    assert!(err1_opt.is_none());
    ...
    let (b2, err2_opt) =
        Buffer::new_from_entire_reader(&mut reader2).expect("EntireReader case 2 failed");
    ...
    impl<'a> Read for ErrorAfterData<'a> {
        fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
            ...
            if self.read_ptr >= self.error_on_read {
            ...
            let max_bytes = std::cmp::min(
                self.error_on_read - self.read_ptr,
                self.data.len() - self.read_ptr,
            );
            let bytes_to_read = std::cmp::min(buf.len(), max_bytes);
            buf[..bytes_to_read]
                .copy_from_slice(&self.data[self.read_ptr..self.read_ptr + bytes_to_read]);
            self.read_ptr += bytes_to_read;
            Ok(bytes_to_read)
        }
    }
    let data3 = b"part1_error_part2".to_vec();
    let mut reader3 = ErrorAfterData {
        data: &data3,
        read_ptr: 0,
        error_on_read: 6,
    }; // Error after "part1_"
    let (b3, err3_opt) =
        Buffer::new_from_entire_reader(&mut reader3).expect("EntireReader case 3 failed");
...
#[test]
fn test_api_new_buffer_random() {
    // Corresponds to Go's TestNewBufferRandom
    ...
    assert!(
        matches!(
            b.with_data_mut(|_| Ok(())),
            Err(MemguardError::ProtectionFailed(_))
        ),
        "Random buffer should be ReadOnly initially"
    );
    assert!(b.is_alive());
    b.destroy().unwrap();
...
#[test]
fn test_api_buffer_freeze_melt() {
    // Corresponds to Go's TestFreeze and TestMelt
    ...
    .expect("Initial write failed, buffer not mutable?");
    ...
    assert!(
        ...
        "Write to frozen buffer should fail or be hard"
    );
...
fn test_api_new_buffer_from_entire_reader_with_file() {
    ...
    let mut file_reader =
        std::fs::File::open(test_file_name).expect("Failed to open test file for reading");
    let (b, err_opt) = Buffer::new_from_entire_reader(&mut file_reader)
        .expect("NewBufferFromEntireReader with file failed");

    assert!(
        err_opt.is_none(),
        "Reading entire file should not produce I/O error option"
    );
    assert_eq!(
        b.size(),
        file_content.len(),
        "Buffer size mismatch with file content"
    );
    b.with_data(|d| {
        assert_eq!(d, file_content);
        Ok(())
    })
    .unwrap();
    assert!(
        ...
        "Buffer from entire reader should be ReadOnly"
    );
...
#[test]
fn test_api_buffer_seal() {
    // Corresponds to Go's TestSeal
    test_setup();
    ...
    opened_b
        .with_data(|d| {
            assert_eq!(d, original_data.as_slice());
            Ok(())
        })
        .unwrap();
...
#[test]
fn test_api_buffer_size_destroy_alive_mutable() {
    // Combines Go's TestSize, TestDestroy, TestIsAlive, TestIsMutable
...
#[test]
fn test_api_buffer_copy_operations() {
    // Corresponds to Go's TestCopy and TestCopyAt
    ...
    b.with_data_mut(|dest| {
        // b currently holds "yellow submarine"
        let offset = 7; // "yellow " <- place "marine" here
        assert!(
            offset + src_data2.len() <= dest.len(),
            "CopyAt would overflow"
        );
    ...
    b.with_data(|d| {
        assert_eq!(d, b"yellow marineine");
        Ok(())
    })
    .unwrap();
    ...
    assert!(matches!(
        b_null.with_data_mut(|_| Ok(())),
        Err(MemguardError::SecretClosed)
    ));
}

#[test]
fn test_api_buffer_move_operations() {
    // Corresponds to Go's TestMove and TestMoveAt
    ...
    assert!(
        src_data2.iter().all(|&x| x == 0),
        "Source for b2 (MoveAt) not wiped"
    );
    b2.with_data(|d| {
        let mut expected = vec![0u8; 16];
        expected[4..4 + original_src_data2_content.len()]
            .copy_from_slice(&original_src_data2_content);
    ...
    assert!(matches!(
        b_destroyed.with_data_mut(|_| Ok(())),
        Err(MemguardError::SecretClosed)
    ));
...
#[test]
#[serial] // Run this test serially to avoid coffer conflicts
fn test_api_new_enclave_random() {
    // Corresponds to Go's TestNewEnclaveRandom
    test_setup();
    ...
    assert_eq!(
        b.size(),
        32,
        "Opened buffer from random enclave has wrong size"
    );
    let is_zeroed = b.with_data(|d| Ok(d.iter().all(|&x| x == 0))).unwrap();
...
#[test]
#[serial] // Run this test serially to avoid coffer conflicts
fn test_api_enclave_open_properties() {
    // Covers parts of Go's TestOpen
    test_setup();
    ...
    assert_eq!(
        e.size(),
        b.size(),
        "Enclave size and opened buffer size mismatch"
    );
    ...
    assert!(
        ...
        "Buffer opened from enclave should be ReadOnly"
    );
    b.destroy().unwrap();
}

#[test]
fn test_api_buffer_equality_operations() {
    // Covers Go's TestEqualTo, TestCopy, TestMove concepts
    ...
    b_copy_dest
        .with_data(|d| {
            assert_eq!(&d[..copy_src_data.len()], copy_src_data);
            Ok(())
        })
        .unwrap();
    ...
    assert!(
        move_src_data.iter().all(|&x| x == 0),
        "Move source not wiped"
    );
    b_move_dest
        .with_data(|d| {
            assert_eq!(d, b"move this");
            Ok(())
        })
        .unwrap();
...
fn test_purge_logic() {
    ...
    println!(
        "TEST: buffer1.destroyed() before purge = {}",
        destroyed_before
    );
    ...
    println!(
        "TEST: buffer1.destroyed() after purge = {}",
        destroyed_after
    );
...
fn test_new_buffer_from_reader() {
    ...
    use std::io::Read; // Already covered // Already covered by use std::io::{Cursor, Read, Write};
