...
mod signals_tests {
    ...
    fn test_catch_signal_integration() {
        ...
        let output = cmd
            .output()
            .expect("Failed to execute subprocess for TestCatchSignal");
        ...
        assert!(
            stderr_str.contains("Subprocess: Custom signal handler called for signal"),
            "Custom signal handler message not found in stderr."
        );
        assert!(
            stderr_str.contains("Subprocess: TCP listener closed by signal handler."),
            "Listener close message not found in stderr."
        );
    }
    ...
    fn test_catch_interrupt_integration() {
        ...
        let output = cmd
            .output()
            .expect("Failed to execute subprocess for TestCatchInterrupt");
        let status = output.status;
        let stdout_str = String::from_utf8_lossy(&output.stdout);
        let stderr_str = String::from_utf8_lossy(&output.stderr);
        ...
        assert!(
            stderr_str.contains("memguard: Interrupt signal"),
            "Default interrupt handler log not found in stderr. Got: {}",
            stderr_str
        );
        assert!(
            stderr_str.contains("caught. Cleaning up..."),
            "Cleanup message not found in stderr. Got: {}",
            stderr_str
        );
    }
}
