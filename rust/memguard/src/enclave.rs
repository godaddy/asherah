...
impl Enclave {
    ...
    pub fn new(data: &mut [u8]) -> Result<Self> {
        if data.is_empty() {
            return Err(MemguardError::OperationFailed("Enclave data must not be empty".into()));
        }
        ...
        let final_ciphertext_result = key_buffer.with_data(|key_bytes| {
            ...
            let unbound_key = match UnboundKey::new(chacha20_poly1305_alg(), key_bytes) {
                Ok(key) => key,
                Err(_) => return Err(MemguardError::OperationFailed("Failed to create encryption key".into())),
            };
            ...
            match getrandom::getrandom(&mut nonce_bytes) {
                Ok(_) => {},
                Err(e) => return Err(MemguardError::OperationFailed(
            ...
            match less_safe_key.seal_in_place_append_tag(nonce, Aad::empty(), &mut data_to_encrypt_and_tag) {
                Ok(_) => {},
        ...
        match final_ciphertext_result {
            Ok(encrypted_data) => {
                ...
                wipe(data);
                Ok(Self { ciphertext: encrypted_data })
            }
            Err(e) => {
    ...
    pub fn new_random(size: usize) -> Result<Self> {
        if size == 0 {
            return Err(MemguardError::OperationFailed("Enclave random size must be positive".into()));
        }
    ...
    pub fn open(&self) -> Result<Buffer> {
        ...
        if self.ciphertext.len() < OVERHEAD {
             return Err(MemguardError::CryptoError("Ciphertext too short to contain data, tag, and nonce.".into()));
        }
        ...
        let coffer_guard = match coffer.lock() {
            ...
            Err(_) => {
                ...
                return Err(MemguardError::OperationFailed("Failed to lock coffer".into()));
            }
        };
        ...
        let decryption_result = key_buffer.with_data(|key_bytes| {
            buffer.with_data_mut(|buffer_data| {
                ...
                if self.ciphertext.len() < NONCE_SIZE { // Should be caught by OVERHEAD check earlier
                    return Err(MemguardError::CryptoError("Ciphertext too short to contain nonce".to_string()));
                }
                let nonce_bytes = &self.ciphertext[..NONCE_SIZE];
                let nonce_array = match <[u8; NONCE_SIZE]>::try_from(nonce_bytes) {
                    Ok(arr) => arr,
                    Err(_) => return Err(MemguardError::CryptoError("Invalid nonce format".to_string())),
                };
                let nonce = Nonce::assume_unique_for_key(nonce_array);
                
                let actual_ciphertext_with_tag = &self.ciphertext[NONCE_SIZE..];
                if actual_ciphertext_with_tag.len() < TAG_SIZE { // TAG_SIZE is the min length for ring AEAD output
                     return Err(MemguardError::CryptoError("Ciphertext (post-nonce) too short".to_string()));
                }

                // Create the decryption key
                let unbound_key = match UnboundKey::new(chacha20_poly1305_alg(), key_bytes) {
                    Ok(key) => key,
                    Err(_) => return Err(MemguardError::CryptoError("Failed to create decryption key".to_string())),
                };
                let less_safe_key = LessSafeKey::new(unbound_key);
                ...
                // Decrypt the data
                let plaintext = match less_safe_key.open_in_place(nonce, Aad::empty(), &mut ciphertext_to_decrypt) {
                    Ok(pt) => pt,
                    Err(_) => return Err(MemguardError::CryptoError("Decryption failed (authentication failed)".to_string())),
                };
...
mod tests {
    ...
    fn test_core_new_enclave() {
        ...
        assert!(data.iter().all(|&x| x == 0), "Original data buffer was not wiped");
        assert_ne!(data, data_original, "Original data should differ after wipe");

        // Verify the length of the ciphertext
        assert_eq!(enclave.ciphertext.len(), data_original.len() + OVERHEAD, "Ciphertext has unexpected length");
    ...
    fn test_core_seal_and_open() {
        ...
        assert_eq!(enclave.ciphertext.len(), original_buffer_data.len() + OVERHEAD, "Ciphertext length after seal is incorrect");
        ...
        opened_buffer.with_data(|decrypted_data| {
            assert_eq!(decrypted_data, original_buffer_data.as_slice(), "Decrypted data does not match original");
            Ok(())
        }).unwrap();
        ...
        opened_buffer.destroy().expect("Destroy for opened_buffer failed");
    }
    ...
    fn test_core_open_modified_ciphertext() {
        ...
        match enclave.open() {
            Err(MemguardError::CryptoError(msg)) => {
                assert!(msg.contains("Decryption failed") || msg.contains("authentication failed"), "Expected decryption error, got: {}", msg);
            }
            Ok(opened_buffer) => {
                opened_buffer.destroy().ok(); // Clean up if open unexpectedly succeeded
                panic!("Opening modified ciphertext should fail");
            }
            Err(e) => panic!("Unexpected error type when opening modified ciphertext: {:?}", e),
        }
    }

    #[test]
    fn test_core_enclave_size() {
        let enclave = Enclave { ciphertext: vec![0u8; 1234] }; // Direct construction for test
