...
impl Coffer {
    ...
    pub fn new() -> Result<Self> {
        ...
        thread::spawn(move || {
            ...
            while !rekey_thread_destroyed_signal.load(Ordering::Relaxed) {
                ...
                if rekey_thread_destroyed_signal.load(Ordering::Relaxed) { break; } // Check again after sleep
                ...
                match coffer_mutex.try_lock() {
                    Ok(coffer_guard) => {
                        if coffer_guard.destroyed() { // Check if coffer itself was destroyed by another thread
                            break;
                        }
    ...
    pub(crate) fn rekey(&self) -> Result<()> {
        ...
        let right_hash_current = self.right.with_data(|right_data| {
            Ok(hash(right_data))
        })?;
        ...
        let right_hash_new = self.right.with_data(|right_data| {
            Ok(hash(right_data))
        })?;
...
mod tests {
    ...
    impl Coffer {
        ...
        fn internal_hashes_for_test(&self) -> Result<([u8; 32], [u8; 32])> {
            if self.destroyed() { return Err(MemguardError::SecretClosed); }
    ...
    fn test_coffer_new() {
        ...
        assert!(!coffer.destroyed(), "Coffer should not be destroyed after new");
        ...
        let left_is_zero = coffer.left.with_data(|d| Ok(d.iter().all(|&x| x == 0))).unwrap();
        assert!(!left_is_zero, "left buffer is all zeros after init");

        let right_is_zero = coffer.right.with_data(|d| Ok(d.iter().all(|&x| x == 0))).unwrap();
    ...
    fn test_coffer_init() {
        let coffer = new_test_coffer();
        let view1_data = coffer.view().unwrap().with_data(|d| Ok(d.to_vec())).unwrap();
        coffer.init().expect("Coffer re-init failed");
        let view2_data = coffer.view().unwrap().with_data(|d| Ok(d.to_vec())).unwrap();
        assert_ne!(view1_data, view2_data, "Coffer value should change after init");
        coffer.destroy().expect("Coffer destroy failed");
        assert!(matches!(coffer.init(), Err(MemguardError::SecretClosed)), "init on destroyed coffer should fail with SecretClosed");
    }
    ...
    fn test_coffer_view() {
        ...
        view.with_data(|data| {
            ...
            assert!(!data.iter().all(|&x| x == 0), "View data should not be all zeros");
            Ok(())
        }).expect("Failed to access view data");
        view.destroy().expect("View destroy failed");

        coffer.destroy().expect("Coffer destroy failed");
        assert!(matches!(coffer.view(), Err(MemguardError::SecretClosed)), "view on destroyed coffer should fail with SecretClosed");
    }
    ...
    fn test_multiple_views_same_key() {
        ...
        let bytes1 = view1.with_data(|data| Ok(data.to_vec())).expect("Could not read from first view");
        let bytes2 = view2.with_data(|data| Ok(data.to_vec())).expect("Could not read from second view");
        
        assert_eq!(bytes1, bytes2, "Key views should contain the same key material before rekey");
    }
    ...
    fn test_coffer_rekey() {
        ...
        let view1 = coffer.view().expect("Could not create first view before re-keying");
        let bytes1 = view1.with_data(|data| Ok(data.to_vec())).expect("Could not read from first view");
        let (left_hash_before, right_hash_before) = coffer.internal_hashes_for_test().expect("Failed to get internal hashes before rekey");
        
        coffer.rekey().expect("Re-keying failed");
        
        let view2 = coffer.view().expect("Could not create second view after re-keying");
        let bytes2 = view2.with_data(|data| Ok(data.to_vec())).expect("Could not read from second view");
        let (left_hash_after, right_hash_after) = coffer.internal_hashes_for_test().expect("Failed to get internal hashes after rekey");
        
        assert_eq!(bytes1, bytes2, "Key value should be the same even after re-keying");
        
        // Check that the internal representation changed
        assert_ne!(left_hash_before, left_hash_after, "Coffer.left internal data should change after rekey");
        ...
        assert_ne!(right_hash_before, right_hash_after, "Coffer.right internal data should change after rekey");
        coffer.destroy().expect("Coffer destroy failed");
        assert!(matches!(coffer.rekey(), Err(MemguardError::SecretClosed)), "rekey on destroyed coffer should fail with SecretClosed");
    }
    ...
    fn test_coffer_destroy() {
        ...
        assert!(coffer.destroyed(), "Coffer should be marked destroyed after destroy");
        
        // After destroy, operations should fail
        assert!(matches!(coffer.view(), Err(MemguardError::SecretClosed)), "view on destroyed coffer should fail");
        assert!(matches!(coffer.rekey(), Err(MemguardError::SecretClosed)), "rekey on destroyed coffer should fail");
