...
type Loader interface {
	...
	Load(ctx context.Context, key string) (*DataRowRecord, error)
}

// Storer declares the behavior for storing data in a persistence store.
type Storer interface {
	// Store persists the DataRowRecord using the specified key.
	Store(ctx context.Context, key string, d DataRowRecord) error
}
