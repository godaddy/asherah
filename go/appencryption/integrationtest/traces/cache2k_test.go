...
func TestRequestORMNight(t *testing.T) {
	for _, p := range policies {
		p := p
		t.Run(p, func(t *testing.T) {
			t.Parallel()
			...
			testRequest(t, NewCache2kProvider, opt,
				"trace-mt-db-*-night.trc.bin.bz2", "request_ormnight-"+p+".txt")
		})
	}
}

func TestSizeORMNight(t *testing.T) {
	for _, p := range policies {
		p := p
		t.Run(p, func(t *testing.T) {
			t.Parallel()
			...
			testSize(t, NewCache2kProvider, opt,
				"trace-mt-db-*-night.trc.bin.bz2", "size_ormnight-"+p+".txt")
		})
	}
}
